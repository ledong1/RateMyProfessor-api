...
def get_professor_info(name):
    name.replace("%20"," ")
    SJU.SearchProfessor(name)
    return jsonify(
    {
	   'overall_rating' : SJU.professorlist[SJU.indexnumber]["overall_rating"],
       'link' : "www.ratemyprofessors.com/ShowRatings.jsp?tid=" + str(SJU.professorlist[SJU.indexnumber]["tid"])
    }
    )
